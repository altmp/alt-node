...
// Strategy for loading a altv builtin CommonJS module that isn't
// through normal resolution
translators.set('alt', async function moduleStrategy(url) {
  // Slice 'alt:' scheme
  const id = StringPrototypeSlice(url, 4);
  const _exports = process._linkedBinding('alt').getResourceExports(id);

  return new ModuleWrap(url, undefined, Object.keys(_exports), function() {
    debug(`Loading alt resource ${url}`);

    for (const exportName in _exports) {
      // We might trigger a getter -> dont fail.
      let value;
      try {
        value = _exports[exportName];
      } catch {}
      this.setExport(exportName, value);
    }
  });
